...
import { drawGrid, toAxis } from "@/src2/p5/graphics";
import { SketchProps } from "react-p5/@types";
import { isBound } from "../model/solver/operation/node/effectives/effective";
import { Vertex, VertexId } from "../model/solver/operation/vertex/vertex";
import { getCurrentGraph, getVertex, updateCurrentGraph } from "../model/useStore";
...
export function touchMoved() {
  ...
  if (dragData.dragging) {
    if (dragData.panning) {
      ...
      logger.debug({ centerX, centerY, panStart, originalCenter });
