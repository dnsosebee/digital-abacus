import { genNodeId } from "../../../../schema/node";
import { UPDATE_MODE } from "../../../settings";
import { mainGraph } from "../../../store";
import { CircuitPosition, NodeEdge } from "../../edges/nodeEdge";

const MULTIPLIER_ID = "multiply";
const SQUARE_ROOT_ID = "sqrtsqrt";

export const GEOMETRIC_MEAN_CONFIG_JSON = {
  primitive: false as const,
  subgraph: {
    edges: [
      {
        vertices: [
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: MULTIPLIER_ID, handle: 0 },
            dragging: false,
            hidden: false,
            selected: false,
          },
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: MULTIPLIER_ID, handle: 1 },
            dragging: false,
            hidden: false,
            selected: false,
          },
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: MULTIPLIER_ID, handle: 2 },
            dragging: false,
            hidden: false,
            selected: false,
          },
        ],
        id: MULTIPLIER_ID,
        selected: false,
        operator: { primitive: true, type: "multiplier", bound: 2 },
        hidden: false,
        position: { x: 127.22915649414062, y: 194.84895706176758 },
        label: "",
      },
      {
        vertices: [
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: SQUARE_ROOT_ID, handle: 0 },
            dragging: false,
            hidden: false,
            selected: false,
          },
          {
            value: { x: 0.5, y: 0, delta: { x: 0, y: 0 } },
            id: { node: SQUARE_ROOT_ID, handle: 1 },
            dragging: false,
            hidden: false,
            selected: false,
          },
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: SQUARE_ROOT_ID, handle: 2 },
            dragging: false,
            hidden: false,
            selected: false,
          },
        ],
        id: SQUARE_ROOT_ID,
        selected: false,
        operator: {
          primitive: false,
          boundArray: [2],
          interfaceVertexIds: [
            { node: "log00000", handle: 1 },
            { node: "multiply", handle: 1 },
            { node: "exponent", handle: 1 },
          ],
          subgraph: {
            edges: [
              {
                vertices: [
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "log00000", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "log00000", handle: 1 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                ],
                id: "log00000",
                selected: false,
                operator: { primitive: true, type: "exponential", bound: 0 },
                hidden: false,
                position: { x: 147.78583226354175, y: 175.59003854461594 },
                label: "",
              },
              {
                vertices: [
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "multiply", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 0.5, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "multiply", handle: 1 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "multiply", handle: 2 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                ],
                id: "multiply",
                selected: false,
                operator: { primitive: true, type: "multiplier", bound: 2 },
                hidden: false,
                position: { x: 375.2633608974397, y: 184.51072594202378 },
                label: "",
              },
              {
                vertices: [
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "exponent", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "exponent", handle: 1 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                ],
                id: "exponent",
                selected: false,
                operator: { primitive: true, type: "exponential", bound: 1 },
                hidden: false,
                position: { x: 455.6024225425043, y: 495.0893784132097 },
                label: "",
              },
              {
                vertices: [
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "log00000", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "multiply", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                ],
                id: "wire0000",
                selected: false,
                source: { node: "log00000", handle: 0 },
                target: { node: "multiply", handle: 0 },
                primaryLeft: true,
              },
              {
                vertices: [
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "multiply", handle: 2 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                  {
                    value: { x: 0, y: 0, delta: { x: 0, y: 0 } },
                    id: { node: "exponent", handle: 0 },
                    dragging: false,
                    hidden: false,
                    selected: false,
                  },
                ],
                id: "wire1111",
                selected: false,
                source: { node: "multiply", handle: 2 },
                target: { node: "exponent", handle: 0 },
                primaryLeft: true,
              },
            ],
            mode: 1,
            focus: null,
          },
        },
        hidden: false,
        position: { x: 183.22915649414062, y: 492.8489570617676 },
        label: "aᵇ",
      },
      {
        vertices: [
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: MULTIPLIER_ID, handle: 2 },
            dragging: false,
            hidden: false,
            selected: false,
          },
          {
            value: { x: 1, y: 0, delta: { x: 0, y: 0 } },
            id: { node: SQUARE_ROOT_ID, handle: 0 },
            dragging: false,
            hidden: false,
            selected: false,
          },
        ],
        id: "KVmCVkEP",
        selected: false,
        source: { node: MULTIPLIER_ID, handle: 2 },
        target: { node: SQUARE_ROOT_ID, handle: 0 },
        primaryLeft: true,
      },
    ],
    mode: 1,
    focus: null,
  },
  boundArray: [2],
  interfaceVertexIds: [
    {
      node: MULTIPLIER_ID,
      handle: 0,
    },
    {
      node: MULTIPLIER_ID,
      handle: 1,
    },
    {
      node: SQUARE_ROOT_ID,
      handle: 2,
    },
  ],
};

export const addGeometricMean = (position: CircuitPosition) => {
  const id = genNodeId();
  const vertices = [
    mainGraph.addFree(1, 0, { node: id, handle: 0 }),
    mainGraph.addFree(1, 0, { node: id, handle: 1 }),
    mainGraph.addFree(1, 0, { node: id, handle: 2 }),
  ];
  const edge = new NodeEdge(
    vertices,
    GEOMETRIC_MEAN_CONFIG_JSON,
    UPDATE_MODE,
    id,
    position,
    false,
    false,
    "geo. mean(a,b)"
  );
  mainGraph.edges.push(edge);
};
